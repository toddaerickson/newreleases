...
- **Zero Goodreads candidates = breakage, not a quiet week.** `run()` sets `scraper_alarm` and exits 1 *at the very end*, after the shortlist/catalog/email are written — so a Goodreads outage still delivers what StoryGraph found, but the Actions run goes red instead of committing an empty shortlist and staying green. StoryGraph's own zero-result case only logs (its browse coverage is genuinely patchy).
